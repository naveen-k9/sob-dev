...
export default function CategoryBrowserScreen() {
  ...
  const mealTimeCategories = useMemo(
    () => categories.filter((c) => c && c.group === "meal-time" && c.isActive),
    [categories]
  );

  const collectionCategories = useMemo(
    () => categories.filter((c) => c && c.group === "collection" && c.isActive),
    [categories]
  );

  const allCategories: Category[] = useMemo(() => {
    const all = [...mealTimeCategories, ...collectionCategories];
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
            ...
            {activeOffers.map((offer) => (
              <OfferCard
                ...
                offer={{
                  ...
                  validUntil:
                    offer.validUntil ?? new Date(offer.validTo).toDateString(),
                }}
          ...
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitleSmall}>
              {activeCategoryId ? "Products" : "Pick a category"}
            </Text>
            {activeCategoryId && (
