...
interface DeliveryOrder {
  ...
  quantity: number;
  deliveryTime: string;
  status: 'packaging' | 'packaging_done' | 'delivery_started' | 'reached' | 'delivery_done';
...
export default function DeliveryDashboard() {
  ...
  const recalcStats = useCallback((orders: DeliveryOrder[]) => {
    ...
    const earnings = orders.reduce((sum, o) => sum + (typeof o.orderValue === 'number' ? o.orderValue : 0), 0);
    setStats({ totalDeliveries: total, pendingDeliveries: pending, completedDeliveries: completed, totalEarnings: `₹${earnings}` });
  }, []);
    ...
    try {
      ...
      const orders: DeliveryOrder[] = subs
        ...
        .map((s: Subscription) => {
          ...
          const addr = (u?.addresses?.find?.((a: Address) => a.id === s.addressId) || u?.addresses?.[0]);
          const addOnIds = (s.additionalAddOns?.[todayStr] ?? []) as string[];
          const addOnSuffix = addOnIds.length > 0 ? ` + ${addOnIds.length} add-on(s)` : '';
          return {
            id: s.id,
            customerName: u?.name ?? 'Customer',
            customerPhone: u?.phone ?? '',
            address: addr ? `${addr.addressLine}, ${addr.city} - ${addr.pincode}` : 'N/A',
            ...
            paymentStatus: (s.paidAmount ?? 0) >= s.totalAmount ? 'paid' : 'pending',
          };
        });
  ...
  const handleStatusUpdate = (orderId: string, currentStatus: DeliveryOrder['status']) => {
    ...
    Alert.alert(
      ...
      [
        ...
        {
          text: 'Confirm',
          onPress: () => updateOrderStatus(orderId, nextStatus as DeliveryOrder['status']),
        },
      ]
    );
  };
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'packaging_done': return AlertCircle;
      case 'delivery_started': return Truck;
      case 'reached': return Navigation;
      case 'delivery_done': return CheckCircle;
  ...
  const getActionButtonText = (status: string) => {
    switch (status) {
      case 'packaging': return 'Packaging Done';
      case 'packaging_done': return 'Start Delivery';
      case 'delivery_started': return 'Reached';
      case 'reached': return 'Delivery Done';
            ...
            <View style={styles.sectionHeader}>
              ...
              <View style={styles.headerButtonsRow}>
                <TouchableOpacity
                  ...
                  onPress={() => {
                    const count = deliveryOrders.filter(o => o.status === 'packaging').length;
                    if (count === 0) {
                      Alert.alert('Nothing to update', 'No orders in packaging state.');
                      return;
                    }
                    Alert.alert(
                      ...
                      [
                        { text: 'Cancel', style: 'cancel' },
                        {
                          ...
                          onPress: () => {
                            setDeliveryOrders(prev => {
                              const next = prev.map(o => o.status === 'packaging' ? { ...o, status: 'packaging_done' as const } : o);
                              recalcStats(next);
                              return next;
                            });
                          }
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    if (count === 0) {
                      Alert.alert('Nothing to update', 'No orders ready to start delivery.');
                      return;
                    }
                    Alert.alert(
                      ...
                      [
                        { text: 'Cancel', style: 'cancel' },
                        {
                          ...
                          onPress: () => {
                            setDeliveryOrders(prev => {
                              const next = prev.map(o => o.status === 'packaging_done' ? { ...o, status: 'delivery_started' as const } : o);
                              recalcStats(next);
                              return next;
                            });
                          }
                ...
                >
                  <Text style={styles.bulkButtonText}>All Delivery Started</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={onRefresh} style={styles.iconButton} testID="btn-refresh">
                ...
                <View key={order.id} style={styles.orderCard}>
                  <View style={styles.orderHeader}>
                    <View style={styles.customerInfo}>
                      ...
                      <Text style={styles.mealName}>{order.mealName} (x{order.quantity})</Text>
                      <Text style={styles.deliveryTime}>Delivery: {order.deliveryTime}</Text>
                    </View>
                  ...
                  {order.status !== 'delivery_done' ? (
                    <SwipeAdvance
                      ...
                      onComplete={() => handleStatusUpdate(order.id, order.status)}
                  ...
                  <View style={styles.orderActions}>
                    ...
                    {order.status !== 'delivery_done' && (
                      ...
                      >
                        <StatusIcon size={16} color="white" />
                        <Text style={styles.actionButtonText}>{getActionButtonText(order.status)}</Text>
                      </TouchableOpacity>
                    )}
...
function SwipeAdvance({
  ...
  const reset = useCallback(() => {
    Animated.spring(translateX, { toValue: 0, useNativeDriver: false, bounciness: 8, speed: 12 }).start();
  }, [translateX]);

  const complete = useCallback(() => {
    Animated.timing(translateX, { toValue: maxX, duration: 160, useNativeDriver: false }).start(() => {
      try {
        onComplete();
      } catch (e) {
  ...
  const panResponder = useRef(
    PanResponder.create({
      onMoveShouldSetPanResponder: (_, gesture) => {
        const should = Math.abs(gesture.dx) > 5 && Math.abs(gesture.dy) < 20 && gesture.dx > 0;
        return should;
      },
      onPanResponderGrant: () => {
        setIsDragging(true);
  ...
  return (
    <View style={s.container} onLayout={onLayout} testID={testID}>
      ...
      >
        <Animated.View style={[s.fill, { width: Animated.add(translateX, new Animated.Value(knobSize + padding * 2)), opacity: isDragging ? 0.9 : 0.8 }]} />
