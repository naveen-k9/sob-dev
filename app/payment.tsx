...
type Params = Partial<
  Record<
    "amount" | "description" | "name" | "email" | "contact" | "ackOrderId",
    string | string[]
  >
>;

export default function PaymentScreen() {
  const { amount, description, name, email, contact, ackOrderId } =
    useLocalSearchParams<Params>();

  const first = useCallback(
    (v: string | string[] | undefined): string | undefined => {
  ...
  const handleThankyouUrl = (url: string) => {
    try {
      ...
      const nextId =
        ackIdStr && typeof ackIdStr === "string"
          ? ackIdStr
          : createdOrderIdRef.current ?? "unknown";
      if (status === "success") {
        router.replace({
