  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      if (mode === 'signin') {
        ...
        const res = await emailSignUp({ email: email.trim().toLowerCase(), password, name: name.trim() });
        if (!res.success) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.content}>
        <View style={styles.form}>
          <Text style={styles.title}>{mode === 'signin' ? 'Welcome back' : 'Join us'}</Text>
...
const styles = StyleSheet.create({
  ...
  inputRow: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 10, paddingHorizontal: 12, backgroundColor: '#FFFFFF' },
  input: { flex: 1, paddingVertical: 14, paddingHorizontal: 10, fontSize: 16, color: '#111827' },
  button: { backgroundColor: '#111827', paddingVertical: 16, borderRadius: 10, marginTop: 8 },
  buttonDisabled: { opacity: 0.6 },
  buttonText: { color: 'white', textAlign: 'center', fontSize: 16, fontWeight: '600' },
  linkText: { textAlign: 'center', color: '#111827', marginTop: 12, fontSize: 14, fontWeight: '600' },
});
