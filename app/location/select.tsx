...
export default function LocationSelectScreen() {
  ...
  const { setActiveAddress, setCurrentLocationAddress, isAddressActive } =
    useActiveAddress();

  // Auth context for user addresses
  const { user, updateUserAddresses, addAddress: addAddressToUser } = useAuth();
  ...
  const [mapRegion, setMapRegion] = useState<Region>({
    latitude: 17.385, // Hyderabad coordinates as shown in images
    longitude: 78.4867,
  ...
  const getDistanceText = (): string => {
    ...
    if (distance < 1000)
      return `${Math.round(distance)} meters away from current location`;
  ...
  const handleSearchResultSelect = async (
    ...
    try {
      ...
      if (data.result && data.result.geometry) {
        ...
        console.log(
          "Completed polygons:",
          polygons.filter((p) => p.completed).length
        );
        console.log(
          "Serviceable polygons for place:",
          serviceablePolygons.length
        );
        ...
        if (!isServiceable) {
          console.log(
            "Location not serviceable, showing notify modal in 300ms"
          );
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      if (reverseGeocode.length > 0) {
        ...
        console.log(
          "Completed polygons:",
          polygons.filter((p: Polygon) => p.completed).length
        );
  ...
  const handleMapRegionChange = (region: Region) => {
    ...
    console.log(
      "Map center serviceable polygons found:",
      serviceablePolygons.length
    );
  ...
  const handleSaveAddress = async () => {
    if (!selectedLocation || !houseNumber.trim()) {
      Alert.alert("Missing Information", "Please fill in house/flat number.");
      return;
    }
    ...
    try {
      ...
      // Use the new addAddress method which handles Firebase sync
      const newAddress = await addAddressToUser(addressData);
  ...
  const handleSubmitNotifyRequest = async () => {
    ...
    try {
      ...
      const { createServiceAreaNotificationRequest } = await import(
        "@/services/firebase"
      );
      ...
      // Save to Firebase
      const requestId = await createServiceAreaNotificationRequest(
        notifyRequest
      );
          ...
          >
            ...
            {searchResults.map((result, index) => (
              <TouchableOpacity
                ...
                onPress={() =>
                  handleSearchResultSelect(result.place_id, result.description)
                }
              >
        ...
        <View style={styles.bottomActions}>
          ...
          <TouchableOpacity
            style={[
              ...
              isSavingAddress && styles.disabledButton,
            ]}
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.notifyInfo}>
            ...
            <Text style={styles.notifyInfoDescription}>
              We don't deliver to this location yet, but we're expanding! Leave
              your details and we'll notify you when delivery is available in
              your area.
            </Text>
          </View>
          ...
          <View style={styles.formSection}>
            ...
            <Text style={styles.notifyDisclaimer}>
              We'll contact you as soon as delivery service is available in your
          ...
          <View style={styles.centerMarker}>
            <View
              style={[
                styles.pinContainer,
                {
                  backgroundColor: selectedLocation?.isServiceable
                    ? "#34C759"
                    : "#FF3B30",
                },
              ]}
                ...
                <View style={styles.locationTextInfo}>
                  ...
                  <Text
                    style={[
                      ...
                      {
                        color: selectedLocation.isServiceable
                          ? "#34C759"
                          : "#FF3B30",
                      },
                    ]}
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    if (showSearchDrawer) {
                      ...
                      setTimeout(() => {
                        console.log(
                          "Opening notify modal after search drawer closes"
                        );
                        setShowNotifyModal(true);
                      }, 300);
                    } else {
